...
public class AppFrame extends JFrame
     ...
     * RF-001: El sistema debe permitir contar la cantidad de elementos por
     * categoria e imprimirlos en la barra de tareas.
     * Ejemplo: Total Items: 06; Package: 02, Class: 02; Component: 02
     */
    private void updateItemCount() {
        if (getCurrentEditor() == null) return;
        java.util.List<DiagramElement> children =
                getCurrentEditor().getDiagram().getChildren();
        int packages = 0, classes = 0, components = 0;
        for (DiagramElement elem : children) {
            if (elem instanceof PackageElement)   packages++;
            else if (elem instanceof ClassElement)     classes++;
